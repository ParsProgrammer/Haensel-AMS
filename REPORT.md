...
The CLI in `src/attribution_pipeline/cli.py` wires those tasks together and exposes optional time-range arguments. The package also defines a console script, `haensel-attribution`, via `pyproject.toml`. The database remains the system of record: API results are written to `attribution_customer_journey`, then the final reporting table is rebuilt from SQLite and exported to CSV.
...
- `channel_reporting.cost` is interpreted as the total marketing cost for all sessions on the given date and channel. IHC and IHC revenue are calculated from attributed customer journey sessions and then joined to the total spend aggregation.
...
The IHC API documentation describes a POST endpoint at `https://api.ihc-attribution.com/v1/compute_ihc` with `conv_type_id` as a URL parameter and an API key in the `x-api-key` header. The API response contains a `value` list with `conversion_id`, `session_id`, and `ihc`. The public documentation mentions 100 customer journeys and 3000 sessions per request, but the free test account returned a stricter 200-session limit. The pipeline therefore chunks requests on both journey count and a conservative 200-session request size. Transient HTTP responses such as 429 and 5xx are retried with exponential backoff.

## Validation

The code includes focused pytest coverage for customer journey construction, API chunking, and channel reporting aggregation. The pipeline also validates that IHC attribution sums to 1.0 for each conversion before completing.

## What Could Be Improved

- Persist raw API responses for auditability.
